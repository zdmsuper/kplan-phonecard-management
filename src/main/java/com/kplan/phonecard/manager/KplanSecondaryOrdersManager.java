...
//							list.add(cb.or(cb.isNull(r.get("logistics_info")),cb.notEqual(r.get("logistics_info"), "恶意订单")));
						...
						if(query.getDomain().getLogistics_info().equals("6")) {
							list.add(cb.equal(r.get("logistics_info"), "待开户订单"));
						}
					}
...
//						list.add(cb.like(r.get("post_city"), "%成都%"));
				...
				if(managerInfo.getBasicUserInfo().getCardId()!=null&&managerInfo.getBasicUserInfo().getCardId().contains("成华")) {
					list.add(cb.or(cb.equal(r.get("order_source"), "GZ")));
		...
		try {
			...
			if (l != null && l.size() > 0) {
				...
				if ("1".equals(proctype)) {
					...
					if("CD".equals(ordersource)||"CDOFFDITE".equals(ordersource)) {
						...
						}else if(order.getLogistics_info().contains("待开户")) {
